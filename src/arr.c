...
Option _array_init(Allocator a, size_t mem_sz, size_t def_sz) {
	...
	arr.members = a.alloc(&a, def_sz * mem_sz);
	if(!arr.members)
...
Error array_resize(Array_t *arr, size_t nsz) {
	...
	if(nsz == arr->size)
		return ERROR_SUCC;
	arr->members = arr->allocator.realloc(&arr->allocator, arr->members, nsz*arr->mem_sz);
	arr->size = nsz;
	if(!arr->members)
		return ERROR_FAIL;
	return ERROR_SUCC;
...
void array_debuginfo(Array_t arr) {
	printf("(Array_t(mem_sz=%lu, size=%lu, used=%lu, allocator=(size=%lu, used=%lu))) [", arr.mem_sz, arr.size, arr.used, arr.allocator.size, arr.allocator.used);
	for(size_t i = 0; i < arr.used; i++) {
		printf("%u", *(unsigned *)NTH_A(arr, i));
		if(i != arr.used-1)
			printf(", ");
